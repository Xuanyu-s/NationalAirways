...
if (confirm == JOptionPane.YES_OPTION) {
 auditLogs.log("LOGOUT", "Staff logged out.");
    this.dispose();
    if (parent != null) {
    ...
    if (confirm == JOptionPane.YES_OPTION) {
        ...
        auditLogs.log("CALL_PASSENGER", "Called Queue #" + next.getQueueNumber() + " (" + next.getFullName() + ")");
}
...
storage.recordAuditLog("Called passenger \"" + next.getFullName()
    ...
    // FIX: Removed duplicate/nested choice branches and tied directly to auditLogs.log()
    if (choice == 0) {
        storage.updateBoardingStatus(current.getQueueNumber(), "Boarded");
        auditLogs.log("PASSENGER_BOARDED", "Marked Queue #" + current.getQueueNumber() + " (" + current.getFullName() + ") as Boarded.");
    } else if (choice == 1) {
        ...
        if (reason != null) {
            String finalReason = reason.isBlank() ? "N/A" : reason;
            storage.updateBoardingStatus(current.getQueueNumber(), "No-Show");
            storage.setNoShowReason(current.getQueueNumber(), finalReason);
            auditLogs.log("PASSENGER_NOSHOW", "Marked Queue #" + current.getQueueNumber() + " (" + current.getFullName() + ") as No-Show. Reason: " + finalReason);
        }
    }
